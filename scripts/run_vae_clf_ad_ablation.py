...
# -*- coding: utf-8 -*-
"""
run_vae_clf_ad_ablation.py

Variante de ablación de canales del pipeline VAE+clasificador para AD/CN.

- Usa el ConvolutionalVAE oficial de betavae_xai.models.convolutional_vae
- Habilita `--channels_to_use` y `--fast_ablation` para exploraciones rápidas.
- Resultados usados en el paper "Explainable Latent Representation Learning for Alzheimer's Disease: ..."
"""
from __future__ import annotations
from pathlib import Path
import sys

# Ruta raíz del repo = carpeta padre de scripts/
...
if SRC_DIR.exists():
    ...
    print(f"[WARN] No se encontró el directorio src en {SRC_DIR}")
...
import subprocess
from betavae_xai.models.classifiers import get_classifier_and_grid, get_available_classifiers
import joblib
...
def load_data(tensor_path: Path, metadata_path: Path) -> Tuple[
    Optional[np.ndarray],
    ...
    try:
        # allow_pickle en caso de que los nombres vengan como objetos Python
        ...
        # === NUEVO: nombres de ROIs y etiquetas de red dentro del .npz ===
        ...
        if network_labels_in_order is not None:
            ...
            logger.info(f"Se leyeron {len(network_labels_in_order)} etiquetas de red de 'network_labels_in_order'.")
        else:
    ...
    if len(merged_df) < num_tensor_subjects:
        logger.warning(
            f"Algunos SubjectIDs del tensor ({num_tensor_subjects}) no se encontraron en los metadatos. "
            f"Merged: {len(merged_df)}."
        )

    # === NUEVO: devolvemos también roi_names_in_order y network_labels_in_order ===
...
def train_and_evaluate_pipeline(global_tensor_all_channels: np.ndarray, 
    ...
    for fold_idx, (train_dev_clf_idx_in_cn_ad_df, test_clf_idx_in_cn_ad_df) in enumerate(outer_cv_clf.split(X_classifier_subject_indices_in_cn_ad_df, stratify_key_for_clf_cv)):
        ...
        # ▼▼▼ Estratificación robusta para el split de validación del VAE (igual que en FULL) ▼▼▼
        ...
        try:
            stratify_key_vae_split = temp_vae_strat_df.apply(
        ...
        if args.vae_val_split_ratio > 0 and len(global_indices_vae_training_pool) > 10:
            try:
                vae_actual_train_indices_local_to_pool, vae_internal_val_indices_local_to_pool = sk_train_test_split(
                    ...
                    stratify=stratify_key_vae_split,  # Usamos la clave robusta de estratificación del VAE
...
if __name__ == "__main__":
    ...
    group_data.add_argument(
        "--output_dir",
        type=str,
        default="./vae_clf_output_ablation_v1.8.0",
        help="Directorio para guardar resultados de ablación."
    )
    group_data.add_argument(
        "--channels_to_use",
        type=int,
        nargs='*',
        default=None,
        help="Lista de índices de canales a usar (0-based, según DEFAULT_CHANNEL_NAMES)."
    )
    ...
    if getattr(args, 'fast_ablation', False):
        ...
        args.beta_vae = max(getattr(args, 'beta_vae', 1.0), 1.2)
    ...
    if global_tensor_data is not None and metadata_df_full is not None:
        ...
        if roi_names_in_order is not None:
            if len(roi_names_in_order) != n_rois_tensor:
                logger.warning(
                    f"Número de ROIs del tensor ({n_rois_tensor}) "
                    f"≠ len(roi_names_in_order) ({len(roi_names_in_order)})."
                )

            roi_joblib_path = Path(args.output_dir) / f"roi_order_{n_rois_tensor}.joblib"
            joblib.dump(roi_names_in_order, roi_joblib_path)
            logger.info(f"Lista de {len(roi_names_in_order)} ROIs guardada en: {roi_joblib_path}")
        else:
            logger.warning(
                "No se encontraron 'roi_names_in_order' en el .npz; "
                "no se guardará roi_order_*.joblib para la ablación."
            )
